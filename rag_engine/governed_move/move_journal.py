"""Durable single-file MOVE journal (Roadmap v7 Phase E4).

Journals live only at ``{persist_dir}/governed_single_file_move_journal/{operation_id}.json``.
...
JOURNAL_DIR_NAME = "governed_single_file_move_journal"

PHASE_PREPARED = "PREPARED"
PHASE_REGISTRY_PREPARED = "REGISTRY_PREPARED"
PHASE_FILESYSTEM_MOVED = "FILESYSTEM_MOVED"
...
class MoveJournalError(ValueError):
    """Raised when MOVE journal preconditions fail."""
...
def validate_operation_id(operation_id: str) -> str:
    ...
    if not isinstance(operation_id, str) or not operation_id.strip():
        raise MoveJournalError("operation_id is required")
    op = operation_id.strip()
    if "/" in op or "\\" in op:
        raise MoveJournalError("operation_id must be a safe opaque identifier")
    if any(ord(c) < 0x20 or ord(c) == 0x7F for c in op):
        raise MoveJournalError("operation_id must not contain control characters")
    if op == ".." or op.startswith("../") or op.endswith("/.."):
        raise MoveJournalError("operation_id must not contain traversal segments")
...
def _require_absolute_path(raw: str, field: str) -> str:
    if not isinstance(raw, str) or not raw.strip() or "\0" in raw:
        raise MoveJournalError(f"{field} must be a non-empty absolute path")
    path = Path(raw)
    if not path.is_absolute():
        raise MoveJournalError(f"{field} must be an absolute path")
...
def validate_persist_dir_path(persist_dir: str | Path) -> str:
    ...
    if not persist_entry.is_absolute():
        raise MoveJournalError("persist_dir must be an absolute path")
    if persist_entry.is_symlink():
        raise MoveJournalError("persist_dir must not be a symlink")
    persist = persist_entry.resolve()
    if not persist.is_dir():
        raise MoveJournalError("persist_dir must exist")
    return str(persist)
...
def journal_root_dir(persist_dir: str | Path, *, create_root: bool = False) -> Path:
    ...
    persist = persist_entry.resolve()
    if not persist.is_dir():
        raise MoveJournalError("persist_dir must exist before MOVE journal access")
    root_entry = persist / JOURNAL_DIR_NAME
    if create_root:
        root_entry.mkdir(parents=True, exist_ok=True)
    elif not root_entry.is_dir():
        raise MoveJournalError("MOVE journal directory not found")
    if root_entry.is_symlink():
        raise MoveJournalError("journal root must not be a symlink")
...
def read_journal_exact(persist_dir: str | Path, operation_id: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise MoveJournalError(f"MOVE journal not found for operation_id {operation_id!r}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise MoveJournalError(f"MOVE journal unreadable: {exc}") from exc
    if not isinstance(payload, dict):
        raise MoveJournalError("MOVE journal payload must be a JSON object")
    return payload
...
def create_prepared_journal(
    *,
    ...
    if path.exists():
        raise MoveJournalError(
            f"MOVE journal already exists for operation_id {operation_id!r}"
        )
    record: dict[str, Any] = {
...
def advance_journal_phase(
    ...
    *,
    ...
    if phase not in EXECUTION_PHASES and phase not in TERMINAL_PHASES:
        raise MoveJournalError(f"invalid journal phase: {phase!r}")
    try:
    ...
    *,
    ...
    for field, value in expected.items():
        ...
        if not isinstance(bound, str) or Path(bound).resolve() != Path(value).resolve():
            raise MoveJournalError(f"journal binding mismatch for {field}")
