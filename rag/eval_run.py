...
from rag.query import answer, retrieve
...
def _sources_ok(sources: list[dict], substrs: list[str]) -> bool:
    if not substrs:
        return True
    blob = " ".join(
        f"{s.get('source', '')} {s.get('collection', '')}" for s in sources
    )
...
def run_case(case: dict, *, retrieval_only: bool) -> dict:
    ...
    if retrieval_only:
        # Negatives: empty retrieval is a pass; positives need hits (+ optional hint)
        if expect_refuse:
            result["pass"] = True  # retrieval-only cannot judge LLM refuse
            result["note"] = "retrieval-only: LLM refuse not checked"
        else:
            result["pass"] = bool(docs) and result["scope_filter_ok"] and result["source_hint_ok"]
        return result

    text, ans_sources = answer(q, scope=scope, k=5)
    result["answer_preview"] = text[:400]
    result["sources"] = ans_sources[:5]
    result["scope_filter_ok"] = _scope_ok(ans_sources, scope)
    result["refused"] = _looks_like_refuse(text)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--retrieval-only",
        action="store_true",
        help="Skip LLM; check retrieval/scope only (usable mid-reindex)",
    )
    parser.add_argument(
        "--ids",
        nargs="*",
